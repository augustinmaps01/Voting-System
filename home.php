				        		...
				        		<?php
									...
									while($row = $query->fetch_assoc()){
										...
										while($crow = $cquery->fetch_assoc()){
											$sql = "SELECT party_list.Partylist FROM candidates LEFT JOIN party_list ON party_list.id = candidates.party_list_ID WHERE position_id = '".$row['id']."' AND candidates.id = '".$crow['id']."'";
										...
										if(empty($partylist)){
											$candidate .= '
											<li>
												'.$input.'<button type="button" class="btn btn-primary btn-sm btn-flat clist platform" data-platform="'.$crow['platform'].'" data-fullname="'.$crow['firstname'].' '.$crow['lastname'].'"><i class="fa fa-search"></i> Platform</button><img src="'.$image.'" height="100px" width="100px" class="clist"><span class="cname clist">'.$crow['firstname'].' '.$crow['lastname'].'
												  </span>  
